    ...
    public GraphDemoFrame() {
        ...
        buttonDfsIterator.addActionListener(e -> {
            try {
                ...
                showSystemOut(() -> {
                    int[] vertexes = new int[graph.vertexCount()];
                    for (int from = 0; from < graph.vertexCount(); from++) {
                        ...
                        for (int vertex : vertexes) {
                            if (vertex == 0) {
                                System.out.println("граф не является связным");
                                return;
                            } else {
                                vertex = 0;
                            }
                    ...
                    System.out.println("граф является связным");
                });
            } catch (Exception exc) {
                SwingUtils.showErrorMessageBox(exc);
            }
        ...
        buttonDfsStack.addActionListener(e -> {
            try {
                ...
                showSystemOut(() -> {
                    int[] vertexes = new int[graph.vertexCount()];
                    for (int from = 0; from < graph.vertexCount(); from++) {
                        GraphAlgorithms.dfs(graph, from, v -> vertexes[v] = 1);
                        for (int vertex : vertexes) {
                            if (vertex == 0) {
                                System.out.println("граф не является связным");
                                return;
        ...
        buttonDfsRecursion.addActionListener(e -> {
            try {
                ...
                showSystemOut(() -> {
                    int[] vertexes = new int[graph.vertexCount()];
                    for (int from = 0; from < graph.vertexCount(); from++) {
                        GraphAlgorithms.dfsRecursion(graph, from, v -> vertexes[v] = 1);
                        for (int vertex : vertexes) {
                            if (vertex == 0) {
                                System.out.println("граф не является связным");
                                return;
        ...
        buttonDfsQueue.addActionListener(e -> {
            try {
                ...
                showSystemOut(() -> {
                    int[] vertexes = new int[graph.vertexCount()];
                    for (int from = 0; from < graph.vertexCount(); from++) {
                        GraphAlgorithms.bfs(graph, from, v -> vertexes[v] = 1);
                        for (int vertex : vertexes) {
                            if (vertex == 0) {
                                System.out.println("граф не является связным");
                                return;
